...
集中维护 JAX 的 Shadowrocket、Clash Verge / Mihomo、OpenClash / Mihomo、自用 Cloudflare 节点方案，以及 OpenWrt / ImmortalWrt 主路由运维知识。**GitHub `main` 是本仓库唯一正式版本。** 聊天记录、截图、旧 commit、旧附件和文档示例都不能替代 `main` 中的当前实际文件。
...
版本号、端口、DNS、TUN、策略组、Provider、上游 commit、OpenWrt 接口/服务/包管理器等会变化的信息，**必须读取对应正式文件或实际运行环境的最新内容**，不要从本 README 或聊天历史中推断。
...
├─ openwrt/            R2S / ImmortalWrt 系统运维知识
...
- `openwrt/docs/`：OpenWrt / ImmortalWrt 专项知识、教程、排障与历史。
...
2. 以实际配置文件或运行环境为权威值，不用文档快照覆盖动态字段。
3. 最小修改，不无关重构，不擅自改组名、路由语义、DNS/IPv6/TUN 边界。
4. OpenWrt / ImmortalWrt 优化遵守“先测基线 → 单项修改 → 复测 → 回滚/保留”。
5. 修改后做语法/引用/安全检查，并重新读取 GitHub 实际结果。
6. 用户要求提交时，提交后报告修改文件、验证结果和 commit；未获提交授权时遵守 `AGENTS.md` 的 Git 安全规则。
