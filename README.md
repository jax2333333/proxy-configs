...
集中维护 JAX 的三套代理配置。后续以本仓库 `main` 分支作为主要维护版本，原 Gist 保留作历史来源/备份。
...
- 本仓库目前为 **Public**，禁止提交真实机场订阅 URL、Token、密码、Cookie、API Key 等敏感信息。
...
三套配置可以独立修改，也可以进行跨客户端规则同步。涉及 Apple、AI、TikTok、YouTube、Microsoft、Telegram、Netflix、DNS、节点筛选等公共逻辑时，先判断三个客户端的语法差异，再分别落地，避免机械复制导致兼容问题。
