...
- 当前核心入口是 `🎯 节点选择`，默认选中 `A|智能选择`，并保留 `B|智能选择` 用于机场级手动切换。
- `A|智能选择` / `B|智能选择` 分别对 A/B 前缀的本机场全部节点执行 `url-test`；当前前缀匹配兼容实际节点命名 `A | 节点名` / `B | 节点名`，同时也兼容无空格的 `A|节点名` / `B|节点名`。香港、日本、新加坡、美国、台湾再各自提供 A/B 地区智能组做区域优化。
- `✋ 手动选择` 展示当前订阅实际节点，不固定某个机场具体节点名。
- 当前应用策略组包括 `✨ AI服务`、`🎵 TikTok`、`▶️ YouTube`、`🐙 GitHub`、`✈️ Telegram`、`🍿 流媒体`、`🌍 国际服务`；应用组优先提供 `🎯 节点选择`，再提供适用的 A/B 地区智能、`A|智能选择` / `B|智能选择` 和 `✋ 手动选择`。
...
- Apple 保持直连优先；当前移动配置中 `🎯 节点选择`、`A|智能选择`、`B|智能选择`、A/B 地区智能、`✨ AI服务`、`▶️ YouTube`、`🐙 GitHub`、`✈️ Telegram`、`🎵 TikTok`、`🍿 流媒体`、`🌍 国际服务`、`✋ 手动选择` 等名称视为稳定接口，修改前必须以 `main` 实际配置为准。
