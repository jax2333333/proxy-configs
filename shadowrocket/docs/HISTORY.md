...
## 2026-09-25：V6.6 → V6.7.1 双机场 A/B 分组

- V6.6 将香港、日本、新加坡、美国、台湾地区智能拆成 A/B 两套。
- V6.7 将原跨机场的 `🧠 智能选择` 拆成 `A|智能选择` / `B|智能选择`，`🎯 节点选择` 默认使用 A 机场智能组，同时保留 B 机场作为手动切换入口。
- 首版来源判断按无空格 `A|` / `B|` 设计，但设备实际节点命名为 `A | 地区 序号` / `B | 地区 序号`；本次实测样本为 `B | 日本 01`，因此出现智能组为空。
- V6.7.1 将来源判断修正为允许竖线两侧空格的 `^A\s*\|\s*` / `^B\s*\|\s*`，同时兼容无空格旧写法。
- 后续修改订阅重命名、A/B 前缀或地区正则时，必须同时验证机场级智能组与地区智能组是否仍能命中实际节点名称。
- 本次修复未改变 DNS、IPv6、WebRTC、TikTok / 抖音规则顺序与 Home Clean 职责边界。
