...
当用户询问“我现在可以做什么”“下一步有什么操作”“给我操作关键词”或同类问题时：

1. 读取 `shadowrocket/docs/OPERATIONS-KEYWORDS.md`；
2. 同时检查当前 GitHub `main`、相关配置 / Toolkit / CI / 规则 Fork 的实际状态；
3. 只输出当前真正有意义的 3～8 个操作关键词，不机械复述完整清单；
4. 每个关键词使用独立代码块，方便手机端直接复制；
5. 用户随后直接发送关键词，即按该关键词进入对应操作，不要求重新解释背景。

授权语义固定为：

- `检查` / `体检` / `排查` / `查看` / `推荐` → 默认只读；
- `同步` / `优化` / `更新` / `建立` / `添加` / `修复` → 用户主动发送该关键词，视为对**该关键词明确范围**的一次性修改授权；
- 关键词不是永久授权，也不能扩展到其它目录、其它配置或其它仓库；
- 所有写操作仍必须在写入前重新读取 `main` 最新目标文件，并在写入后重新读取实际结果、检查 CI / diff / 安全边界。
