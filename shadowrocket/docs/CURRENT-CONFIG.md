...
- DNS 与 DNS 防绕行基础；
- 国内 / 国外规则分流；
- `🎯 节点选择`、`A|智能选择` / `B|智能选择`、A/B 地区智能，以及 AI、TikTok、YouTube、GitHub、Telegram、流媒体、国际服务等当前策略组；
- 实际代理节点选择；
- Toolkit 广告/隐私/脚本增强。
...
- `A|智能选择` / `B|智能选择` 分别使用 `url-test` 自动测速本机场 A/B 前缀的全部节点；前缀识别必须兼容实际命名 `A | 节点名` / `B | 节点名`，并同时兼容 `A|节点名` / `B|节点名`。机场级智能不设置地区关键词；地区归类和区域优化由各 A/B 地区智能组单独负责。
- `🎯 节点选择` 通过 `policy-select-name=A|智能选择` 默认选中 `A|智能选择`；需要切换机场时直接选择 `B|智能选择`，不固定任何具体机场节点名。
- 应用策略组以 `🎯 节点选择` 作为主入口，并按服务需要提供 A/B 地区智能、`A|智能选择` / `B|智能选择` 与 `✋ 手动选择`。
