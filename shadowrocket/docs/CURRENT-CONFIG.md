...
`proxy-stability.sgmodule`（`block-quic = all-proxy`）与 `webrtc-privacy.sgmodule` 属于可选实验增强，默认不写死到主配置；出现兼容性问题直接关闭对应模块做 A/B。

## 修改前检查

读取真实配置后检查 `[General]`、`[Rule]` 顺序、`JAX Overrides`、TikTok/抖音先后关系、`FINAL`、DNS、IPv6、UDP/QUIC、目标策略组、远程 RULE-SET 可访问性、模块冲突和 MITM hostname 合并方式。
