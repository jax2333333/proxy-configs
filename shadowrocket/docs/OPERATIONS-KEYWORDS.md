...
- `A|智能选择` / `B|智能选择` 分别对 `A|` / `B|` 前缀的本机场全部节点执行 `url-test`；
...
比较 `A|智能选择` / `B|智能选择` 与香港、日本、新加坡、美国、台湾的 A/B 地区智能组：确认机场级智能分别只覆盖对应 `A|` / `B|` 前缀全部节点，地区智能只做地区归类；检查中英文地名、机场常见缩写和互斥条件，避免跨机场或跨区误收。默认不额外排除 0.01 / 0.1 倍、免费或试用节点，除非用户明确要求。
