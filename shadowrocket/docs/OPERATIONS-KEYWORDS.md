...
- `A|智能选择` / `B|智能选择` 分别对 A/B 前缀的本机场全部节点执行 `url-test`；前缀检测需同时兼容 `A | 节点名` / `B | 节点名` 与无空格 `A|节点名` / `B|节点名`；
- 香港、日本、新加坡、美国、台湾等地区智能组继续按 A/B 来源分别负责地区过滤和区域优选；
- `🎯 节点选择` 通过 `policy-select-name=A|智能选择` 默认选中 `A|智能选择`，同时保留 `B|智能选择` 作为机场级切换入口；
- 应用策略组优先提供 `🎯 节点选择`，再按服务需要提供 A/B 地区智能、`A|智能选择` / `B|智能选择` 与 `✋ 手动选择`；
- 修改后不得改变 TikTok / 抖音规则顺序、Apple 直连优先、Home Clean 边界。

`地区智能正则全面检测`

比较 `A|智能选择` / `B|智能选择` 与香港、日本、新加坡、美国、台湾的 A/B 地区智能组：确认机场级智能分别只覆盖对应 A/B 前缀全部节点，并验证 `A |` / `B |` 带空格与 `A|` / `B|` 无空格两种命名均可命中；地区智能只做地区归类；检查中英文地名、机场常见缩写和互斥条件，避免跨机场或跨区误收。默认不额外排除 0.01 / 0.1 倍、免费或试用节点，除非用户明确要求。
