...
- 不把机场具体节点名硬编码为 `⚙️ 手动选择`、`♻️ ALL`、`🤖️ 人工智能` 的默认节点；
- `⚙️ 手动选择` 展示当前订阅实际节点；
- `♻️ ALL` 与 `🤖️ 人工智能` 继续依赖 `url-test + policy-regex-filter` 自动选择；
- `🚀 策略选择` 可以继续固定选择内部稳定策略组 `♻️ ALL`，因为它是 JAX 自己维护的策略组名，不是机场节点名；
- 修改后不得改变 TikTok / 抖音规则顺序、Apple 直连优先、Home Clean 边界。

`统一智能节点过滤`

比较 `♻️ ALL` 与 `🤖️ 人工智能` 当前过滤口径；只有在用户明确执行该关键词时才统一倍率 / 免费 / 试用排除规则。不得根据旧聊天自动实施。
