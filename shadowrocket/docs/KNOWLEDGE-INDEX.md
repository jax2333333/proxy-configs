...
| 修改主配置 | `CURRENT-CONFIG.md` + `main` 中的 `Jax-shadowrocket-v6.conf` |
| 修改 JAX 自托管规则 | `CURRENT-CONFIG.md` + 当前 `rules/*.list` + 主配置对应 RULE-SET |
...
| FaceTime / Google Meet / Discord 语音 / 网页视频会议异常 | **P0：先读 `TROUBLESHOOTING.md` → WebRTC / STUN** + `CURRENT-CONFIG.md` + 主配置当前 `[General]` + `webrtc-privacy.sgmodule`；先排查 WebRTC Privacy，再查 QUIC / DNS / 节点 |
| 抖音 / TikTok 广告问题 | `HISTORY.md` + `TROUBLESHOOTING.md` + 当前专项 module/script |
| YouTube 去广告 | `CURRENT-CONFIG.md` + 当前 YouTube module/script |
| DNS / IPv6 / QUIC / WebRTC | `TROUBLESHOOTING.md` + 主配置当前 `[General]` + 对应可选 Toolkit 模块 |
