...
| `tiktok-douyin-adblock.module` | TikTok 安全优先；拦截高置信度字节广告域名与明确 `/api/ad/`，并过滤抖音 Feed / 短剧强标记广告对象 | `*.amemv.com`、`aweme.snssdk.com` |
...
`tiktok-douyin-adblock.module` 属于干预程度较高的专项模块。当前 v5 对 `*.amemv.com` 与精确的 `aweme.snssdk.com` 做 HTTPS response 脚本处理，并对同一范围内明确的 `/api/ad/` 路径直接 REJECT；`aweme.snssdk.com` 来自 2026-09-06 实测，**不允许扩展成 `*.snssdk.com`**。v5 同时取消 `ad_info/adInfo` 的单独强广告标记资格，新增 `aweme_raw_ad/awemeRawAd`，降低普通作品误删风险。若抖音出现观看历史、推荐流、短剧、搜索、评论或账号页异常，优先关闭本模块做 A/B，不要先扩大 MITM 或封锁整个 ByteDance 共享域名。详细实测与 GitHub 横向对比见 `docs/HISTORY-DOUYIN-ADBLOCK-20260906.md`。
